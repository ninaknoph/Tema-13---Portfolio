...
import { handleImage, handleParagraphs, handleImagePath } from "./utils.js";
...
function init() {
  ...
  burgerIcon.addEventListener("click", () => {
    ...
    burgerIcon.classList.toggle("close-mobile-menu");
  });
...
async function getPost(pageValue) {
  ...
  console.log(result[0].mainImage.asset._ref);
  //project.append(handleImage(result[0].mainImage.asset._ref));
  const cover = document.querySelector(".cover-project");
  cover.src = handleImagePath(result[0].mainImage.asset._ref);
  ...
  team.innerText = result[0].team;

  const delivery = document.querySelector(".delivery");
  delivery.innerText = result[0].delivery;

  const time = document.querySelector(".time");
  time.innerText = result[0].time;

  const research = document.querySelector(".research");
  research.innerText = result[0].research;
  research.append(handleParagraphs(result[0].research));

  const useranalysis = document.querySelector(".useranalysis");
  useranalysis.innerText = result[0].useranalysis;
  useranalysis.append(handleParagraphs(result[0].useranalysis));

  const personas = document.querySelector(".personas");
  personas.innerText = result[0].personas;
  personas.append(handleParagraphs(result[0].personas));

  const concept = document.querySelector(".concept");
  concept.innerText = result[0].concept;
  concept.append(handleParagraphs(result[0].concept));

  const userstory = document.querySelector(".userstory");
  userstory.innerText = result[0].userstory;
  userstory.append(handleParagraphs(result[0].userstory));

  const flowchart = document.querySelector(".flowchart");
  flowchart.innerText = result[0].flowchart;
  flowchart.append(handleParagraphs(result[0].flowchart));

  const moodboard = document.querySelector(".moodboard");
  moodboard.innerText = result[0].moodboard;
  moodboard.append(handleParagraphs(result[0].moodboard));

  const bodyText = document.querySelector(".body-text");

  bodyText.append(handleParagraphs(result[0].body));
}
...
async function getPosts() {
  ...
  const worksList = document.querySelector(".wrapper");
  ...
  result.forEach((post) => {
    ...
    workBlock.classList.add("container"); // vi legger til en class til <a class="work"></a>
    ...
    workCover.setAttribute(
      "src",
      `${cdnUrl}${cover[1]}-${cover[2]}.${cover[3]}`
    );
